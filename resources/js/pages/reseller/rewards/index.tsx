                ...
                {reward.image ? (
                    ...
                    <div className="flex h-full items-center justify-center px-6 text-center">
                        <div>
                            ...
                            <p className="mt-4 text-sm font-bold tracking-[0.18em] text-gojamu-700 uppercase">
                                Reward GoJamu
                            </p>
                        </div>
                    </div>
            ...
            <div className="space-y-4 p-5">
                <div>
                    <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                            <h2 className="text-xl font-black text-gojamu-950">
                                {reward.name}
                            </h2>
                            <p className="text-herbal-600 mt-1 text-sm">
                                {reward.description ??
                                    'Tukar point dengan hadiah yang tersedia di katalog ini.'}
                            </p>
                        </div>
                        <div className="rounded-2xl bg-kunyit-50 px-3 py-2 text-right">
                            <p className="text-kunyit-800 text-[11px] font-bold tracking-[0.18em] uppercase">
                                Butuh point
                            </p>
                            <p className="mt-1 text-lg font-black text-gojamu-950">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                    <div className="rounded-2xl bg-gojamu-50 p-3">
                        ...
                        <p className="mt-1 text-sm font-black text-gojamu-950">
                            {hasLimitedStock ? `${stock} pcs` : 'Tanpa batas'}
                        </p>
                    </div>
                    <div className="rounded-2xl bg-herbal-50 p-3">
                        ...
                        <p className="mt-1 text-sm font-black text-gojamu-950">
                            {formatDate(reward.start_date)}
                        </p>
                        <p className="text-herbal-600 text-xs">
                            s/d {formatDate(reward.end_date)}
                        </p>
                    </div>
                </div>
                ...
                <div className="space-y-2">
                    <button
                        ...
                        className="hover:bg-kunyit-400 disabled:text-herbal-400 inline-flex w-full items-center justify-center rounded-2xl bg-kunyit-500 px-5 py-3 text-sm font-black text-gojamu-950 shadow-sm shadow-kunyit-100 transition disabled:cursor-not-allowed disabled:bg-herbal-100"
                    >
                    ...
                    {form.errors.reward ? (
                        <p className="text-sm font-medium text-mahakunir-700">
                            {form.errors.reward}
                        </p>
                    ) : null}
                </div>
...
export default function RewardsIndex({
    ...
    return (
        <ResellerLayout title="Point & Reward">
            ...
            <section className="via-gojamu-900 to-herbal-900 rounded-[2rem] bg-gradient-to-br from-gojamu-950 p-6 text-white shadow-xl shadow-gojamu-950/15">
                <div className="flex flex-col gap-6">
                    <div className="text-gojamu-200 flex flex-wrap items-center gap-2 text-xs font-bold tracking-[0.16em] uppercase">
                        <Link
                            href={dashboard.url()}
                        ...
                        <span className="text-kunyit-300">Point & Reward</span>
                    </div>
                </div>
                <div className="mt-5 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                    <div className="max-w-2xl">
                        <p className="text-sm font-bold tracking-[0.22em] text-kunyit-300 uppercase">
                            Reward point
                        </p>
                        <h1 className="mt-2 text-3xl font-black tracking-tight md:text-4xl">
                            Kumpulkan point, tukar hadiah.
                        </h1>
                        <p className="mt-3 max-w-xl text-sm leading-7 text-gojamu-100/90 md:text-base">
                            Point aktif Mas Bro tersimpan di sini. Pilih reward
                            yang tersedia, lalu sistem akan memproses penukaran
                            sesuai stok.
                        </p>
                    </div>
                    ...
                    <div className="grid min-w-0 gap-3 sm:grid-cols-2 lg:min-w-[24rem]">
                        <div className="rounded-3xl bg-white/10 p-4 backdrop-blur">
                            <p className="text-xs font-semibold tracking-[0.18em] text-gojamu-100 uppercase">
                                Point aktif
                            </p>
                            <p className="mt-2 text-3xl font-black text-kunyit-300">
                                {current_points} pt
                            </p>
                        </div>
                        <div className="rounded-3xl bg-white/10 p-4 backdrop-blur">
            ...
            <section className="mt-5">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <p className="text-xs font-bold tracking-[0.18em] text-gojamu-700 uppercase">
                            Katalog reward
                        </p>
                        <h2 className="mt-1 text-2xl font-black text-gojamu-950">
                            Pilih hadiah yang paling worth it
                        </h2>
                    </div>
                </div>

                {rewards.length === 0 ? (
                    <div className="border-gojamu-200 mt-4 rounded-[2rem] border border-dashed bg-white p-8 text-center shadow-sm">
                        <p className="text-lg font-black text-gojamu-950">
                            Belum ada reward
                        </p>
                        <p className="text-herbal-600 mt-2 text-sm">
                            Nanti katalog hadiah muncul di sini. Sabar dikit,
                            sistemnya lagi manis-manisnya 😄
                        </p>
                    </div>
                ) : (
            ...
            <section className="mt-5 grid gap-5 xl:grid-cols-2">
                <article className="rounded-[2rem] border border-gojamu-100 bg-white p-5 shadow-sm shadow-gojamu-100">
                    ...
                    {recent_redemptions.length === 0 ? (
                        <div className="text-herbal-600 mt-4 rounded-3xl bg-gojamu-50 p-5 text-sm">
                        ...
                        <div className="mt-4 space-y-3">
                            {recent_redemptions.map((redemption) => (
                                <div
                                    key={redemption.id}
                                    className="rounded-3xl border border-herbal-100 p-4"
                                >
                                    ...
                                    <div className="text-herbal-600 mt-3 grid gap-2 text-sm sm:grid-cols-2">
                                        <p>
                                            Disetujui:{' '}
                                        ...
                                        <p>
                                            Ditolak:{' '}
                                            {formatDate(redemption.rejected_at)}
                                        </p>
                                    </div>
                                    ...
                                    {redemption.rejected_reason ? (
                                        <p className="bg-mahakunir-50 text-mahakunir-800 mt-3 rounded-2xl p-3 text-sm">
                                            {redemption.rejected_reason}
                                        </p>
                                    ) : null}
                                    ...
                                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                        <div>
                                            <div className="flex flex-wrap items-center gap-2">
                                                ...
                                                <span
                                                    className={`rounded-full px-3 py-1 text-xs font-bold ring-1 ${ledgerTone(ledger.type)}`}
                                                >
                                                    {ledger.points > 0
                                                        ? '+'
                                                        : ''}
